...
# ----------------------------
PROBLEM_TEXT = (
    "A smooth ring at O is pulled by two cables and remains in equilibrium. "
    "Cable OA: F1 = 400 N at 30° above +x. "
    "Cable OB: F2 = 250 N at 135° from +x (left & up). "
    "Find the third force F3 (magnitude and direction) required for equilibrium using the force triangle."
)

st.title("Word Problem")
...
# ----------------------------
KEY_DEFS = {
    "equilibrium": "Net force equals zero (ΣF = 0). For rigid bodies, also ΣM = 0 if moments are involved.",
    "force triangle": "Graphical check for three concurrent forces in equilibrium: head-to-tail polygon closes.",
...
if not st.session_state.s_timer_done:
    ...
    if st.session_state.s_timer_started:
        ...
        st.info(f"⏳ Read carefully & visualize the setup — {mmss} remaining")
        st.caption("Picture the ring at O, the two cable directions, and how F₃ closes the head-to-tail triangle.")
...
# --- Define the angle difference function outside the button/logic block
def ang_diff(a, b):
    # smallest unsigned angle difference
    d = (a - b + 180) % 360 - 180
    return abs(d)

# --- Define the line extraction function outside the main logic block
def extract_lines(canvas_json):
    ...
    for obj in objs:
        if obj.get("type") == "line":
            ...
            ang = math.degrees(math.atan2(-dy_canvas, dx))  # convert to math coords (y up)
            # normalize to (-180, 180]
            if ang <= -180: ang += 360
            if ang > 180: ang -= 360
            L = math.hypot(dx, dy_canvas)
            lines.append({"p1": (x1, y1), "p2": (x2, y2), "angle": ang, "length": L})
    return lines


# --- Main T section (unlocked by session state) ---
if st.session_state.get("unlock_T", False):
    st.header("T — Translate: Draw a diagram (FBD or Force Triangle)")

    # Pull givens for checks (from Study step / problem)
    F1 = 400.0
    th1 = 30.0      # deg CCW from +x
    F2 = 250.0
    th2 = 135.0
    # Expected F3 direction from components
    ...
    F3x, F3y = -Sx, -Sy
    th3 = math.degrees(math.atan2(F3y, F3x))   # expected F3 direction

    st.markdown(
        "You can either **draw** your diagram here or **upload a picture** of your hand sketch. "
        "For this problem, a **Force Triangle** is natural, but an **FBD at the ring** is also acceptable."
    )

    # mode variable is defined here
    mode = st.radio("Choose diagram type:", ["Force Triangle", "Free Body Diagram (FBD)"], horizontal=True)

    up_col, draw_col = st.columns([1, 2], vertical_alignment="top")

    # ---------- Upload path ----------
    with up_col:
        uploaded = st.file_uploader("Or upload a diagram image (PNG/JPG)", type=["png", "jpg", "jpeg"])
        if uploaded:
            st.image(uploaded, caption="Uploaded diagram", use_column_width=True)
            st.info("If you uploaded a diagram, you can skip drawing. Use the checklist below to self-verify labels.")
    ...
    with draw_col:
        if not _canvas_ok:
            ...
            st.caption("Draw **lines** for your forces. You may also annotate outside the canvas if needed.")
    ...
    st.markdown("### Label your forces")
    st.caption("Tell the checker which drawn line is F1, which is F2, and which is F3 (if applicable).")

    # Build selectors for labeling lines
    lines = extract_lines(canvas.json_data) if (_canvas_ok and not uploaded) else []
    line_names = [f"Line {i+1} (angle≈{l['angle']:.1f}°, len≈{l['length']:.0f}px)" for i,l in enumerate(lines)]
    idx_opts = list(range(len(lines)))

    colL1, colL2, colL3 = st.columns(3)
    with colL1:
        f1_sel = st.selectbox("Select F1", options=[None]+idx_opts, format_func=lambda i: "None" if i is None else line_names[i], index=0, key="sel_f1")
    with colL2:
        f2_sel = st.selectbox("Select F2", options=[None]+idx_opts, format_func=lambda i: "None" if i is None else line_names[i], index=0, key="sel_f2")
    with colL3:
        f3_sel = st.selectbox("Select F3", options=[None]+idx_opts, format_func=lambda i: "None" if i is None else line_names[i], index=0, key="sel_f3")

    # This section was causing the NameError when placed outside the 'if unlock_T' block
    st.markdown("### Optional scaling (for triangle only)")
    st.caption("If you drew a **Force Triangle**, we can lightly check the **magnitude ratio**: |F1|:|F2| ≈ 400:250.")
    colS1, colS2 = st.columns(2)
    with colS1:
        ratio_tol_pct = st.slider("Length-ratio tolerance (%)", 5, 40, 20)
    with colS2:
        require_f3 = st.checkbox("Require F3 to be present", value=(mode == "Force Triangle"))


    # --------------------------------------------------------------------------------------------------
    # ---- 📚 Optional Refresher: vectors & FBD (appears BEFORE the check button) ----
    st.markdown("#### 📚 Optional Refresher")
    with st.expander("Need a quick review? Click here for vector or FBD help", expanded=False):

        REFRESHER_LINKS = {
            "Head-to-tail vector addition": [
                "[Graphical vector addition (head-to-tail) — Physics Classroom](https://www.physicsclassroom.com/class/vectors/Lesson-1/Vector-Addition)",
                "[Adding vectors graphically — Youtube](https://www.youtube.com/watch?v=_LrDgGhCTJU)"
            ],
            "Free-Body Diagrams (FBD)": [
                "[Free-Body Diagrams — concise overview (Wikipedia)](https://en.wikipedia.org/wiki/Free_body_diagram)",
                "[Engineering Statics: FBD basics (Youtube)](https://www.youtube.com/watch?v=52R61aSWHg0)"
            ]
        }

        topic = st.radio("Pick a topic:", list(REFRESHER_LINKS.keys()), horizontal=True, key="t_ref_topic")

        if topic == "Head-to-tail vector addition":
            st.markdown(
                "- Draw **F₁**, then place **F₂** so its **tail touches the head of F₁**.  \n"
                "- The **closing side** back to the start gives **−(F₁ + F₂)** = **F₃** for equilibrium.  \n"
                "- Measure directions **CCW from +x-axis** and label angles."
            )
        else:
            st.markdown(
                "- Isolate the **ring** as the body.  \n"
                "- Replace cables with **tension forces** along their directions (pulling **away** from the ring).  \n"
                "- Because the ring is **smooth**, no friction force acts at the contact point."
            )

        st.markdown("**Learn more:**")
        for link in REFRESHER_LINKS[topic]:
            st.markdown(f"- {link}")

        st.caption("_You can replace these with your own course links/videos later._")
    # ---- END refresher ----
    # --------------------------------------------------------------------------------------------------
    ...
    # Check button logic (runs when clicked)
    if st.button("✅ Check my diagram"):

        # If user uploaded, just checklist gate without geometric check
        if uploaded:
            # Minimal confirmation checkboxes
            with st.expander("Self-check checklist", expanded=True):
                ok_labels = st.checkbox("I labeled **F1 (400 N @ 30°)** and **F2 (250 N @ 135°)** correctly on the diagram.")
                ok_dirs  = st.checkbox("The **directions** of F1 and F2 match the problem statement.")
                ok_f3    = st.checkbox("I added **F3** with the correct closing direction (equilibrium).", value=(mode == "Force Triangle"))
            if ok_labels and ok_dirs and (ok_f3 or not require_f3):
                st.success("Looks good — moving on.")
                passed = True
            else:
                st.error("Something is missing. Please verify labels/directions (and F3 if required).")

        else:
            # Need at least F1 and F2 selected
            if f1_sel is None or f2_sel is None:
                st.error("Please select which lines correspond to **F1** and **F2**.")
            elif require_f3 and f3_sel is None:
                st.error("Please also select which line is **F3**.")
            else:
                # Direction checks
                ok_ang1 = ang_diff(lines[f1_sel]["angle"], th1) <= tol_angle
                ok_ang2 = ang_diff(lines[f2_sel]["angle"], th2) <= tol_angle
                msgs = []
                msgs.append(f"F1 angle vs {th1:.1f}° → measured {lines[f1_sel]['angle']:.1f}° : {'✅' if ok_ang1 else '❌'}")
                msgs.append(f"F2 angle vs {th2:.1f}° → measured {lines[f2_sel]['angle']:.1f}° : {'✅' if ok_ang2 else '❌'}")

                if require_f3 and f3_sel is not None:
                    ok_ang3 = ang_diff(lines[f3_sel]["angle"], th3) <= tol_angle
                    msgs.append(f"F3 angle vs {th3:.1f}° → measured {lines[f3_sel]['angle']:.1f}° : {'✅' if ok_ang3 else '❌'}")
                else:
                    ok_ang3 = True  # not required

                # Magnitude ratio check (triangle mode only, if both selected)
                if mode == "Force Triangle" and (f1_sel is not None) and (f2_sel is not None):
                    L1, L2 = lines[f1_sel]["length"], lines[f2_sel]["length"]
                    if L1 > 0 and L2 > 0:
                        drawn_ratio = L1 / L2
                        true_ratio  = F1 / F2  # 400/250 = 1.6
                        pct_err = abs(drawn_ratio - true_ratio) / true_ratio * 100.0
                        ok_ratio = pct_err <= ratio_tol_pct
                        msgs.append(f"Length ratio |F1|:|F2| → drawn {drawn_ratio:.2f} vs true {true_ratio:.2f} (err {pct_err:.1f}%) : {'✅' if ok_ratio else '⚠️'}")
                    else:
                        ok_ratio = True  # nothing to compare
                else:
                    ok_ratio = True
                ...
                if ok_ang1 and ok_ang2 and ok_ang3 and ok_ratio:
                    st.success("Nice! Your diagram’s labels and directions look consistent with the problem.")
                    passed = True
                else:
                    st.warning("Some items need adjustment. Fix the ❌/⚠️ items and re-check.")

    # Gate to next step after successful check
    if "T_done" not in st.session_state:
        st.session_state["T_done"] = False
    if passed:
        st.session_state["T_done"] = True
        # Example: unlock the next section (A — Assign coordinates) when ready
        st.session_state["unlock_A"] = True
        st.success("🎉 **Translate (T)** complete. The next section is now unlocked.")
    else:
        st.info("Complete the check above to finish **Translate (T)**.")
...
if st.session_state.get("I_done", False) or st.session_state.get("unlock_C_next", False):
    ...
    gamma_expected = 180.0 - d_tail              # interior angle in the force triangle (should be 75°)

    # expected F3 from components (for reference / checking)
    ...
    if st.button("Check γ", key="C_btn_check_gamma"):
        ...
        if diff <= tol_g:
            st.success(f"✅ γ = {gamma_guess:.2f}° looks good (expected ≈ {gamma_expected:.2f}°).")
            st.session_state["C_gamma_ok"] = True
            st.session_state["C_gamma_val"] = gamma_guess
        else:
            st.warning(
                f"γ seems off by {diff:.2f}°. Hint: tail-to-tail angle = {d_tail:.1f}°, "
                f"so γ = 180° − {d_tail:.1f}° ≈ {gamma_expected:.1f}°."
            )
    ...
    with st.expander("Trig refresher: Law of Cosines", expanded=False):
        ...
        st.caption ( "Law of Cosine (Youtube)](https://www.youtube.com/watch?v=9CGY0s-uCUE)")
    ...
    if st.button("Check |F₃|", key="C_btn_check_F3"):
        if abs(F3_user - F3_lawcos) <= tol_F3:
            st.success(f"✅ |F₃| ≈ {fmt(F3_user)} N (Law of Cosines: {fmt(F3_lawcos)} N).")
            st.session_state["C_F3_ok"] = True
            st.session_state["C_F3_val"] = F3_user
        else:
            st.warning(
                f"|F₃| doesn’t match. With γ = {gamma_used:.2f}°, "
                f"Law of Cosines gives |F₃| ≈ {fmt(F3_lawcos)} N."
            )
    ...
    with st.expander("Trig refresher: Law of Sines", expanded=False):
        ...
        st.caption ( "Law of Sine (Youtube)](https://www.youtube.com/watch?v=9fS0uA4iLxI)")



    # interior angle between line1 and line3 by Law of Sines
    ...
    st.caption(
        f"(With your numbers) an interior angle between Line 1 and Line 3 from Law of Sines is "
        f"α₁₃ ≈ {alpha_13:.2f}°."
    )

    st.markdown("**Now convert that interior geometry to the actual direction θ₃ (CCW from +x).**")
    st.caption(
        "Hint: F₁ has absolute direction θ₁ = 30°.  \n"
        "Use the triangle interior angles to figure out where F₃ points, "
        "then express that as θ₃ measured from +x (0° to 360°)."
    )

    # --- student input: θ3 in 0–360° ---
    ...
    # normalize both angles to [0, 360) before comparison
    if st.button("Check θ₃", key="C_btn_check_th3"):
        ...
        if abs(diff) <= tol_th3:
            st.success(
                f"✅ θ₃ ≈ {theta3_guess:.2f}° is consistent "
                f"(reference ≈ {th3_norm:.2f}° in 0–360° form)."
            )
            st.session_state["C_dir_ok"] = True
        else:
            st.warning(
                f"θ₃ seems off by {abs(diff):.2f}°. "
                f"For reference, the component method gives θ₃ ≈ {th3_norm:.2f}° (in 0–360°)."
            )
    ...
    if st.session_state["C_gamma_ok"] and st.session_state["C_F3_ok"] and st.session_state["C_dir_ok"]:
        ...
        # If you have a final step or summary, unlock it here:
